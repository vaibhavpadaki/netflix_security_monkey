...
sed -i "s/securitymonkey@<YOURDOMAIN>.com/$email/g" /usr/local/src/security_monkey/env-config/config-deploy.py
...
#mv /usr/local/src/dart /usr/local/src/security_monkey/
#tar -xvf dart.tar
...
chmod 777 /var/log/security_monkey/securitymonkey.log
...
echo "running monkey db upgrade"
monkey db upgrade
...
docker run -e "mail=rrgamanagatti@gmail.com" -e "host=ec2-xx-xxx-xxx-xxx.compute-1.amazonaws.com" d -i -t -p 443:443 -p 5000:5000 "us.gcr.io/iron-foundry-138623/netflix/poc:0.2"/usr/local/src/securitymonkey.sh
